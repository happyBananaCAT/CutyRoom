...
router.post("/add",async(req,res)=>{
    ...
    const insert_sql = "insert into blog (id , title , content , category_id , create_time) values (?,?,?,?,?)"
    let {err,rows} = await db.async.all(insert_sql,[id,title,content,category_id,create_time])
    if(err==null){
        res.send({
            code:200,
            msg:"添加博客成功",
...
router.get("/search",async(req,res)=>{
    ...
    //查询分页数据
    let search_sql = "select `id`,`category_id`,`title`,`create_time`,LEFT(`content`,100) as `content`  from blog "+where_sql_str+" order by create_time desc limit ? , ? "
